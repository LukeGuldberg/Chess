...
Chessboard Agent::apply_move(Chessboard board_state, std::pair<int, int> move) {
    ...
    if (board_state.chessboard.at(move.second).piece) {
        board_state.taken_pieces.push_back(
            std::move(board_state.chessboard.at(move.second).piece.value()));
    }
...
int Agent::evaluate(Chessboard state) {
    ...
    for (auto &tile : state.chessboard) {
        if (tile.piece) {
            int pieceValue = get_piece_value(tile.piece->type);
    ...
    for (auto &tile : state.chessboard) {
        if (tile.piece) {
            std::vector<int> possibleMoves =
                tile.piece->get_possible_moves(state);
            score += possibleMoves.size() * (tile.piece->team_white ? -1 : 1);
        }
    }

    // Pawn structure -> rewrite this to have a vector of vectors for each piece
    for (auto &tile : state.chessboard) {
        if (tile.piece) {
            if (tile.piece->team_white) {
